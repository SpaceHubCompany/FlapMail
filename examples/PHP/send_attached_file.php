...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!isValidEmail($email)) {
        ...
        if (empty($_FILES['file']['tmp_name'])) {
            $error = 'Please select a file before sending.';
        } elseif (($_FILES['file']['size'] ?? 0) > 3 * 1024 * 1024) {
            $error = 'The selected file exceeds the 3 MB limit.';
        } else {
            $payload = $_POST;
            $payload['token'] = $token;
            // format can be 'html' or 'text'
            $payload['format'] = 'html';
            $payload['form_type'] = 'docs';
            $boundary = '----FlapMailBoundary' . md5((string) microtime(true));
            $eol = "
";
            $body = '';
...
<head>
  ...
  <title>Send Attached File - FlapMail PHP Example</title>
...
<body>
  <div class="card">
    <h1>Send Attached File</h1>
    <p>A file submission example. Maximum allowed attachment size is 3 MB.</p>
    ...
    <form method="post" enctype="multipart/form-data">
      ...
      <label for="file">Attachment (max 3 MB)</label><input id="file" name="file" type="file" required />
